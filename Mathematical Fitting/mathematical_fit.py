...
x0_range = [28]
c_range = [0.045, 0.047, 0.05, 0.053, 0.055, 0.057, 0.06] 
d_range = [170, 180, 190, 200, 210] 
beta_range = [0.0447, 0.0448, 0.0449, 0.045, 0.0451, 0.0452, 0.0453, 0.0454, 0.0455]
            ...
            for l in x0_range:
                for m in c_range:
                    for n in d_range:
                        for h in beta_range:
                            # Signal spectrum for a given DM cross section and mass
                            int_func = theoretical_func(x, Hal, i, k, j, l, m, n, h)
...
# gets # of tracks per 0.000001 kg (0.001 g)


#plt.figure()
#plot histogram of lengths per volume (0.0026 kg)
#n1, bins1, _ = plt.hist(lengths_per_vol, 36, #each bin will be approx 10 nm
#         facecolor='plum', edgecolor='black', linewidth=0.5)

#plt.plot([45,45], [0, max(n1)], color = 'black', 
#         linestyle='-.', linewidth = 0.5)

#plt.title('# of measured tracks per 0.000001 kg')

## save as .csv to minimize computational expense
#measured_lengths_df = pd.DataFrame(lengths_per_vol)
#measured_lengths_df.to_csv('halite_measured_tracks_per_0.001g.csv', index=False)


#plotSpectrum(Hal)

#x_bins = np.linspace(1,400,num=len(inplane_filtered))
#print(Hal.dRdx_neutrons(x_bins))

# neutron bkg -- unused
#neutron_interaction_rate = Hal.dRdx_neutrons(x_bins)
...
# ## visualize function with current variables
# 
# #plt.plot(x_bins[:-1], theoretical_func(x, Hal, A, B, sigma, x0, c, d), label='actual theory')
# #plt.legend()
# #plt.plot([45,45],[0,2e8])
# 
# 
...
# 
# # Signal spectrum for a given DM cross section and mass
# int_func = theoretical_func(x, Hal, A, B, sigma, x0, c, d)
# 
# # fit integral function
# f = InterpolatedUnivariateSpline(x[:-1], int_func)  # cubic spline default
# 
...
# plt.plot(x[:-1], int_func, label='actual')
# plt.legend()
# 
# # estimated exposure time
# mineral_age_est = 100
...
#     count_per_area = (count **(2./3)) * scanned_area # convert theoretical track length spectra to tracks/area
#     
#     integral_vals.loc[len(integral_vals.index)] = [bins[i], x_batch[0], x_batch[1], round(count_per_area)] 
#     # tabulate integral vals (counts per bin)
# 
